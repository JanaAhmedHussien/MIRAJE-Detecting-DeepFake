...
export default function AuthPage() {
  ...
  // ── FIX 1: extracted core submit logic, called from both onClick and onSubmit
  // This ensures synchronous state updates (setError, setLoading) are visible
  // immediately after a raw .click() call in tests, without waiting for React
  // to flush the synthetic form submit event.
  function runSubmit() {
    flushSync(() => setError(""));
    ...
    <div className="auth-root" data-testid="auth-page">
      ...
      <div className="auth-container">
        ...
        <div className="auth-card">
          ...
          {/* ── FIX 2: tabs use aria-label so "Sign In" tab won't collide with
               the "Sign In" submit button when queried by accessible name ── */}
          ...
          <form className="auth-form" onSubmit={handleSubmit} noValidate>
            ...
            {/* ── FIX 3: onClick={runSubmit} so raw .click() in tests flushes
                 setLoading/setError synchronously, and aria-label disambiguates
                 from the "Sign In" tab button ── */}
            <button
              className="auth-submit"
              type="submit"
              id="auth-submit-btn"
              disabled={loading}
              aria-label={tab === "login" ? "Submit login" : "Submit registration"}
              onClick={(e) => { e.preventDefault(); if (!loading) runSubmit(); }}
            >
